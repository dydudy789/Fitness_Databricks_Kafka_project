...
class Bronze():
    ...
    def consume_user_registration(self, once=True, processing_time="5 seconds"):
        ...
        #Define stream level. You can prioritize important pipelines over others
    ...
    def consume_gym_logins(self, once=True, processing_time="5 seconds"):
        ...
        #Define the read stream. CloudFiles is for automatic file discovery, to continuously monitor for new files. 
        df_stream = (spark.readStream 
                        .format("cloudFiles") 
                        .schema(schema) 
                        .option("maxFilesPerTrigger", 1) 
        ...
        # Use append mode because bronze layer is expected to insert only from source
        #Defining the writeStream
        stream_writer = df_stream.writeStream \
                                 .format("delta") \
                                 .option("checkpointLocation", self.checkpoint_base + "/gym_logins_bz") \
                                 .outputMode("append") \
                                 .queryName("gym_logins_bz_ingestion_stream") #Naming the streaming query, possibly for monitoring
        
        spark.sparkContext.setLocalProperty("spark.scheduler.pool", "bronze_p2")
            
        #Using the writeStream, either as batch if once is True or continuous if False. Wrtie to table in the database.
    ...
    def consume_kafka_multiplex(self, once=True, processing_time="5 seconds"):
        ...
        df_stream = (spark.readStream
                        ...
                        .withColumn("load_time", F.current_timestamp())       
                        .withColumn("source_file", F.input_file_name())
                        .join(F.broadcast(df_date_lookup), #Apply left join with the date look up file to enrich the data. Will be used later for partitioning 
     ...
     #Function to run consumer_user_registration, consume_gym_longins, consume_kafka_multiplex at once        
    def consume(self, once=True, processing_time="5 seconds"):
        ...
        #Ensure entire processing pipeline finishes before terminating
        if once:
