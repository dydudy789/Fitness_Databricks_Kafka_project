...
class CDCUpserter: #Used to rank id by self.sort_by (e.g. updated timestamp column)
    ...
    def upsert(self, df_micro_batch, batch_id):
        ...
        window = Window.partitionBy(self.id_column).orderBy(F.col(self.sort_by).desc()) #compute the rank
        #Only keep first rank
        ...
        df_micro_batch._jdf.sparkSession().sql(self.merge_query) #performa merge query
...
class Silver():
    ...
    def upsert_users(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        df_delta = (spark.readStream
                         ...
                         .withWatermark("registration_timestamp", "30 seconds") #tolerate data up to 30 seconds delay
    ...
    def upsert_gym_logs(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        df_delta = (spark.readStream
                         ...
                         .dropDuplicates(["mac_address", "gym", "login"]) #Use the unique keys to drop duplicates
                   )
        
        stream_writer = (df_delta.writeStream
                                 .foreachBatch(data_upserter.upsert) #Perform the merge query above. Convert the dataframe to gym_logs_delta temp view and use the view the perform the merge to gym_logs (silver table)
                                 .outputMode("update")
    ...
    def upsert_user_profile(self, once=False, processing_time="15 seconds", startingVersion=0):
        ...
        df_cdc = (spark.readStream
                       ...
                       .dropDuplicates(["user_id", "updated"]) #This won't remove multiple udates for same user if it has different timestamp
                 )
